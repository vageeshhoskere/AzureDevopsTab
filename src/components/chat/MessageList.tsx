...
export function MessageList({ messages, isTyping }: MessageListProps) {
  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-8 text-center px-6 animate-fade-in">
        {/* Illustration */}
        ...
        <div className="flex flex-wrap gap-2.5 justify-center max-w-xl">
          {SUGGESTED_PROMPTS.map(({ label }) => (
            <span
              key={label}
              className="px-4 py-2 bg-ado-surface border border-ado-border rounded-full text-xs text-ado-muted font-medium hover:border-ado-accent hover:text-ado-accent transition-colors cursor-default"
            >
              {label}
            </span>
          ))}
