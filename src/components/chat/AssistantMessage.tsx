...
export function AssistantMessage({ parts, workItems, isStreaming }: AssistantMessageProps) {
  return (
    <div className="flex items-start gap-3 px-4 py-2">
      {/* AI avatar */}
      <div className="w-7 h-7 rounded-full bg-ado-accent/20 flex items-center justify-center flex-shrink-0 mt-0.5">
        <span className="text-xs font-bold text-ado-accent">AI</span>
      </div>

      <div className="flex-1 min-w-0 max-w-[85%]">
        {/* Message body */}
        <div className="bg-ado-surface rounded-2xl rounded-tl-sm px-4 py-3">
          {parts.map((part) => {
